...
module ASTTransform
  ...
  # occupies its original source line, so that backtraces, breakpoints and
  # debugger display are correct by construction — CRuby derives line numbers
  # from physical text position, so placement is our line table.
  #
  ...
  # 3. No loc: pack onto the current line — synthetic code has no source-line
  #    truth to preserve.
  #
  # Multi-line renders advance the cursor by their height; displaced
  ...
  class LineAlignedEmitter
    ...
    # Unparser normalizes some single-line constructs into multi-line form
    # (e.g. modifier-if into if/end), which would push following statements
    # off their lines. When the render is taller than the statement's source,
    # compress it back to one line — verified by re-parse so a statement that
    # cannot be safely single-lined (e.g. containing a heredoc) falls back to
    # its multi-line render and re-anchors after itself.
    ...
    def compress_to_single_line(render)
      ...
      # Both sides parsed without scope context, so lvar/send ambiguity
      # cancels out; equality means the newline join preserved structure.
      Unparser.parse(candidate) == Unparser.parse(render) ? candidate : nil
    rescue Parser::SyntaxError
      nil
    end

    # Statements are unparsed in isolation, losing the surrounding scope's
    # local-variable context; without it, Unparser re-parses identifiers as
    # method calls and its dstr round-trip verification fails. Feed it every
    # local assigned or bound anywhere in the tree — an over-approximation
    # that is safe because it only informs Unparser's re-parse verification.
    def unparse(node)
    ...
    # Emits a container body that may be a bare :ensure/:rescue node (their
    # begin/end context comes from the surrounding def/block/kwbegin, so the
    # keywords must be emitted inline, aligned like statements).
    def emit_body(body)
      case body&.type
    ...
    def container_delimiters(node)
      ...
      # Unparser renders empty blocks with braces, but brace blocks cannot
      # hold rescue/ensure bodies; do/end always can.
