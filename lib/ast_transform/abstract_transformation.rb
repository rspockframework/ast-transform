...
module ASTTransform
  class AbstractTransformation < Parser::AST::Processor
    ...
    # Thunks are framework-owned IR: descend into the body so passes that
    # don't know about thunks still process the wrapped statements. Without
    # this, Processor's handler_missing default would pass the node through
    # opaquely, hiding the body from every later transformation. The token
    # (first child) is not a node and passes through untouched.
